...
<Steps>
  ...
  <Step title="Run">
    ...
    the ability to run.
    So, your stories are rendered, each of them is given their own instance of runtime. And whatever the story script
    says is executed.
...
```
Here's a simplistic view of what it looks like:
```mermaid
  flowchart TB
    subgraph hello
        direction TB
...
# Conclusion
We have established that `tick` the unit of time in metz, and it moves forward only when you ask it to.
...
And when you are back, we will re-visit flows in [Flows-102](/flows-102)!
